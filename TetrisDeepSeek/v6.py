...
class Game:
    ...
    def rotate_piece(self):
        if self.current_piece["index"] == 3:  # Квадрат не вращается
            return
        ...
        for y, row in enumerate(shape):
            for x, cell in enumerate(row):
                if cell:
                    ...
                    if grid_y < 0 or grid_y >= GRID_HEIGHT or grid_x < 0 or grid_x >= GRID_WIDTH:
                        can_lock = False
                        break
