...
# Загрузка звуков (если файлы не найдены, будут использованы заглушки)
class DummySound:
    def play(self): pass

try:
    rotate_sound = mixer.Sound('tetris_data/rotate.wav') if os.path.exists('tetris_data/rotate.wav') else DummySound()
    clear_sound = mixer.Sound('tetris_data/clear.wav') if os.path.exists('tetris_data/clear.wav') else DummySound()
    fall_sound = mixer.Sound('tetris_data/fall.wav') if os.path.exists('tetris_data/fall.wav') else DummySound()
    gameover_sound = mixer.Sound('tetris_data/gameover.wav') if os.path.exists('tetris_data/gameover.wav') else DummySound()
except:
...
class Game:
    def __init__(self):
        ...
        self.particles = []
        self.state = "menu"  # menu, game, gameover, highscores
    ...
    def draw_menu(self):
        """Рисует главное меню с рабочими кнопками"""
        # Фон
        screen.fill(BLACK)
        ...
        # Кнопка "Новая игра"
        pygame.draw.rect(screen, BLUE, self.new_game_rect, border_radius=10)
        pygame.draw.rect(screen, WHITE, self.new_game_rect, 2, border_radius=10)
        start_text = big_font.render("Новая игра", True, WHITE)
        start_rect = start_text.get_rect(center=self.new_game_rect.center)
        screen.blit(start_text, start_rect)
        
        # Кнопка "Рекорды"
        pygame.draw.rect(screen, BLUE, self.scores_rect, border_radius=10)
        pygame.draw.rect(screen, WHITE, self.scores_rect, 2, border_radius=10)
        scores_text = big_font.render("Рекорды", True, WHITE)
        scores_rect = scores_text.get_rect(center=self.scores_rect.center)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if self.state == "menu":
                    ...
                    if self.new_game_rect.collidepoint(mouse_pos):
                        self.state = "game"
                        self.reset_game()
                    elif self.scores_rect.collidepoint(mouse_pos):
                        self.state = "highscores"
        
        return True

    def lock_piece(self):
        """Фиксирует текущую фигуру на игровом поле с проверкой"""
        shape = self.current_piece["shape"]
        color = self.current_piece["color"]
        
        # Проверяем, можно ли зафиксировать фигуру
        can_lock = True
        for y, row in enumerate(shape):
            for x, cell in enumerate(row):
                if cell:
                    ...
                    if grid_y < 0:  # Фигура выше игрового поля
                        can_lock = False
                        break
    ...
    def check_completed_lines(self):
        """Проверяет заполненные линии и обновляет игровое поле"""
        lines_cleared = 0
        for y in range(GRID_HEIGHT):
            if all(self.grid[y]):
                lines_cleared += 1
                ...
                for x in range(GRID_WIDTH):
                    for _ in range(5):
                        ...
                        self.particles.append(Particle(px, py, self.current_piece["color"]))
            ...
            if self.state == "game" and not self.game_over and not self.pause:
                ...
                if self.fall_time >= self.fall_speed:
                    ...
                    if self.check_collision():
                        ...
                        self.lock_piece()
                        fall_sound.play()
                        if not self.game_over and not self.new_piece():
                            self.game_over = True
                            self.add_high_score()
                            self.state = "gameover"
